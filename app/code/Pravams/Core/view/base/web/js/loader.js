        ...
        if(jQuery('.totals.reward').length==0){
            if(window.checkoutConfig != undefined){
                if(window.checkoutConfig.ps_reward_points != undefined){
                    var rewardDiscount = window.checkoutConfig.ps_reward_points;
                    jQuery('.totals.shipping.excl').after('<tr class="totals reward"><th class="mark" scope="row"><span class="label">Reward Discount</span></th><td class="amount"><span class="price">'+rewardDiscount+'</span></td></tr>');
                }
            }
        }
